...
> 目标包版本：`com.unity.pipeline@0.2.0-exp.2`（experimental，API 可能变动）
> 验证项目：`F:\UnityProjects\Test`（Unity 6000.5.0f1，已安装 embedded `com.unity.pipeline@0.2.0-exp.2`）
...
```jsonc
{
  ...
  "versionDefines": [
    ...
    { "name": "com.unity.pipeline", "expression": "[0.2.0-exp.2,0.3.0)", "define": "PI_UNITY_PIPELINE" }
  ]
}
```
...
6. **安装辅助**：`/unity-install` 增加可选步骤——检测项目 Unity 版本 ≥ 6000.0 且
   ...
   `com.unity.pipeline@0.2.0-exp.2`（固定版本，exp 包 API 不稳定，不用浮动范围）
...
| Phase 3 run_tests | 反射调用 `command run_tests`（EditMode、空过滤器）完成阻塞往返，返回 `status=completed`、`summary.total=0`；`mode=all` 空过滤器返回两段合并结果；真实 PlayMode 过滤 `UniGameKit.Asset.Tests.AssetRefTests.Constructor_SetsGuid` 触发域重载（managedGeneration 20→21），pipeline 状态文件 `completed/passed=1`，coordinator SessionState 已清理 |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| pipeline 是 exp 包，`CommandRegistry`/`CommandInfo` API 可能破坏性变更 | 升级即编译错误 | 固定版本 `0.2.0-exp.2`；所有引用集中在单一 executor 文件；versionDefines expression 锁定 `[0.2.0-exp.2,0.3.0)`（§3.1） |
