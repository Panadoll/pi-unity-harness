...
namespace Pi.UnityHarness.Editor
{
    internal static class PiUnityPipelineCommandExecutor
    {
        ...
        // 纯内存态：不跨域重载持久化。beforeAssemblyReload 时逐个以 timeout 错误响应并清空，
        // 因为托管 Task 无法在重载后恢复（对比 PiUnityTestCoordinator 用 SessionState 跨重载）。
        ...
        {
#if PI_UNITY_PIPELINE
            // 幂等包装：确保每个请求最多 complete 一次（防止 catch 路径二次响应）。
            ...
            {
                // completeJson 已幂等包装：若 try 块内已完成响应，此处调用会被忽略。
